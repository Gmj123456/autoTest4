...
class LoginPage(BasePage):
    ...
    def login(self, username, password):
        ...
        # 检查页面是否跳转到了正确的页面
        expected_url = "http://192.168.150.222:3066/dashboard/analysis"  # 替换为实际的登录成功后页面 URL
        if self.driver.current_url != expected_url:
            logging.error(f"登录后未跳转到正确页面，当前 URL: {self.driver.current_url}")
        else:
            logging.info("登录后跳转到了正确页面")